...
const mapToDbSchema = (game: StructuredGameData) => {
    ...
    // Campos como wishlistDelta, followers, tags, owners nao sao mapeados 
    // pois nao existem no esquema atual da tabela 'games'.
...
export async function processSteamScraperJson(rawJson: string, apiKey: string): Promise<StructuredResponse> {
    ...
    try {
        ...
        const edgeFunctionUrl = `https://${PROJECT_ID}.supabase.co/functions/v1/process-steam-scraper-json`;
        ...
        if (!structuredResponse.games || structuredResponse.games.length === 0) {
            ...
            toast.success('Processamento concluido, mas nenhum jogo estruturado foi encontrado.');
        ...
        toast.dismiss(toastId);
        toast.success("Processamento concluido! " + updatedCount + " jogos atualizados, " + createdCount + " jogos criados.");
